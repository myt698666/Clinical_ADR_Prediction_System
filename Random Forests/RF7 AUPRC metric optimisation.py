...
from sklearn.metrics import (roc_auc_score, precision_score, accuracy_score, f1_score, matthews_corrcoef, average_precision_score)
...
def objective(trial, target):
    ...
    # Compute AUPRC instead of AUROC
...
for target in selected_targets:#output_columns:
    ...
    with open(f"Random Forests/Best Models/{target}_best_params_AUPRC.json", "w") as f:
        json.dump(best_params, f, indent=4)

with open("best_params_AUPRC.json", "w") as f:
    json.dump(best_params, f, indent=4)
...
for target in selected_targets:
    ...
    feature_importance_df.to_csv(f"Feature Importance/feature_importance_AUPRC_{target}.csv", index=False)
