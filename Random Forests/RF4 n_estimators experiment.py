...
for target in output_columns:
    ...
    feature_counts = []
    roc_auc_scores = []
    n_estimators_list = list(range(10, 410, 10))  # Iterate n_estimators from 10 to 400 in steps of 10

    for n_estimators in n_estimators_list:
        # Initialize RandomForestClassifier with varying n_estimators
        rf_model = RandomForestClassifier(n_estimators=n_estimators, max_depth=15, min_samples_split=5, 
        ...
        y_prob = rf_model.predict_proba(X_test_selected)[:, 1]  # Get probability of positive class
    ...
    # Save the final model with the best n_estimators
    model_filename = f"saved_models/SOC/random_forest_best_{target}_reduced.pkl"
    joblib.dump(rf_model, model_filename)
    
    # Plot n_estimators vs ROC AUC
    plt.figure(figsize=(10, 5))
    plt.plot(n_estimators_list, roc_auc_scores, marker='o', linestyle='-')
    plt.xlabel("Number of Estimators")
    plt.ylabel("ROC AUC")
    plt.title(f"Impact of n_estimators on ROC AUC for Target: {target}")
    plt.show()
...
results_df.to_csv("Random Forests/Results/n_estimator_results.csv", index=False)
