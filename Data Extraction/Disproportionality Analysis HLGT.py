...
def perform_disproportionality_analysis(data):
    results = data.groupby(['Drug', 'HLGT']).apply(calculate_ror).reset_index()
...
# Load your data
data = pd.read_csv('ADR_Summary/Grouped_Drug_HLGT.csv')
...
soc_results.to_csv('ADR_Summary/HLGT_disproportionality_results.csv', index=False)