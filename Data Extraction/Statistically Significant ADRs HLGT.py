...
# Load the data
file_path = 'ADR_Summary/HLGT_disproportionality_results.csv'
soc_results = pd.read_csv(file_path)

# Step 1: Identify top 5 SOC categories by event_count
top_soc = (
    soc_results.groupby('HLGT')['event_count']
    .sum()
    .nlargest(5)
    .index
)
...
adjusted_results.loc[adjusted_results['HLGT'].isin(top_soc), 'ROR'] *= 1
adjusted_results.loc[adjusted_results['HLGT'].isin(top_soc), 'PRR'] *= 1
adjusted_results.loc[adjusted_results['HLGT'].isin(top_soc), 'CI_lower'] *= 1
adjusted_results.loc[adjusted_results['HLGT'].isin(top_soc), 'event_count'] *= 1
...
output_file = 'ADR_Summary/HLGT_significance_matrix.csv'
...
# Customize the plot
#plt.title("Significance Heatmap (Drug vs HLGT Categories)", fontsize=16)
#plt.xlabel("HLGT Categories")
#plt.ylabel("Drugs")
